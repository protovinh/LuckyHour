...
export default function DrinkDetailScreen() {
  const { id } = useLocalSearchParams(); // Get idDrink from URL
  ...
  useEffect(() => {
    if (id) {
      const fetchDrinkDetails = async () => {
        try {
          const response = await axios.get(`${DrinkDetailAPI}${id}`);
  ...
  // Error handling if no drink is found
  if (!drink) {
    return (
      <View style={styles.container}>
